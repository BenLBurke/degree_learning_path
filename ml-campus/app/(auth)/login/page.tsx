...
export default function LoginPage() {
  ...
  const callbackUrl = searchParams.get('callbackUrl') ?? (isProfessor ? '/admin' : '/dashboard');
        ...
        <p className="text-center text-sm text-gray-500 mt-6">
          {isProfessor ? (
            <>
              ...
              <Link href="/signup?role=professor" className="text-indigo-400 hover:text-indigo-300">
                Create one
              </Link>
            </>
          ) : (
